...
from account.models import User, MainCredential, Topic
from QandA.models import Question, Answer, AnswerRequest
...
def signup(request):
    if request.method == 'POST':
        ...
        if uf.is_valid():
            ...
            return HttpResponseRedirect(reverse('QandA:home'))
...
def profile_page(request, u_id):
    ...
    return render(request, 'User/profile.html', {'selected_user': user})
...
def all_notifs(request):
    ...
    return render(request, 'notifications/notifications.html', {'notifs': all_notifs})
...
def topic_search(request):
    ...
    if way == 'summary':
        return render(request, 'Topic/topic_list.html', {'topics': topics, 'way': 'summary'})
    else:
        return render(request, 'Topic/topic_list.html', {'topics': topics, 'way': 'full', 'selected_user': request.user})
...
def edit_profile_avatar(request):
    if request.method == 'POST':
...
def add_credential(request, c_type):
    ...
    if uf.is_valid():
        ...
        credential.save()
        return render(request, 'User/credential_item.html', {'type': c_type, 'credential': credential})
...
def user_credentials_all(request):
    return render(request, 'User/profile_credential-bar.html', {'selected_user': request.user})


def profile_get_feed(request, u_id, f_type):
    selected_user = User.objects.get(id=u_id)
    if f_type == 'answers':
        return render(request, 'Answer/answers.html', {'answers': selected_user.answer_set})
    elif f_type == 'questions':
        return render(request, 'Question/questions.html', {'questions': selected_user.question_set})
    elif f_type == 'shares':
        return render(request, 'Answer/answers.html', {'answers': selected_user.shares})
    elif f_type == 'followings':
        return render(request, 'User/user_list.html', {'users': selected_user.followees})
    elif f_type == 'followers':
        return render(request, 'User/user_list.html', {'users': selected_user.followers})
    elif f_type == 'usertopics':
        return render(request, 'Topic/topic_list.html', {'topics': selected_user.topics, 'way': 'full', 'selected_user':selected_user})
    elif f_type == 'bookmarks':
        if request.user == selected_user:
            return render(request, 'Answer/answers.html', {'answers': selected_user.bookmarks})
        ...
        if request.user == selected_user:
            ...
            return render(request, 'Question/questions.html', {'user_questions': best_questions, 'questions': last_questions})
        ...
        if request.user == selected_user:
            ...
            return render(request, 'Answer/answers.html', {'user_answers': best_answers, 'answers': top_answers})
    elif f_type == 'addtopics':
        if request.user == selected_user:
            return render(request, 'Topic/all-topics.html', {'topics': Topic.objects.all()})
    elif f_type == 'requests':
        if request.user == selected_user:
            requests = AnswerRequest.objects.filter(askee=selected_user)
            return render(request, 'Request/requests.html', {'requests': requests})
